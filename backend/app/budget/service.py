...
async def get_all(conn: Connection, projet_id: str) -> list[dict]:
    rows = await conn.fetch(
        "SELECT id::text, projet_id::text, date, categorie, description, montant, assigne, date_creation "
...
async def create(conn: Connection, projet_id: str, data: dict) -> dict:
    row = await conn.fetchrow(
        """
        INSERT INTO budget_lignes (id, projet_id, date, categorie, description, montant, assigne)
        VALUES ($1::uuid, $2::uuid, $3::date, $4, $5, $6, $7)
        RETURNING id::text, projet_id::text, date, categorie, description, montant, assigne, date_creation
        """,
...
async def update(conn: Connection, ligne_id: str, data: dict) -> dict | None:
    row = await conn.fetchrow(
        ...
        SET date=$2::date, categorie=$3, description=$4, montant=$5, assigne=$6
        WHERE id=$1::uuid
        RETURNING id::text, projet_id::text, date, categorie, description, montant, assigne, date_creation
        """,
