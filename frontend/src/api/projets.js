import client from './client'

export const getProjets          = ()              => client.get('/projets/')
export const createProjet        = (data)          => client.post('/projets/', data)
export const updateProjet        = (id, d)         => client.put(`/projets/${id}`, d)
export const updateStatutProjet  = (id, statut)    => client.patch(`/projets/${id}/statut`, { statut })
export const updateProjetSettings = (id, payload)  => client.patch(`/projets/${id}/settings`, payload)
export const updateProjetPages   = (id, pages)     => client.patch(`/projets/${id}/pages`, { pages_visibles: pages })
export const deleteProjet        = (id)            => client.delete(`/projets/${id}`)
export const cloturerProjet      = (id)            => client.post(`/projets/${id}/cloturer`)
export const reactiverProjet     = (id)            => client.post(`/projets/${id}/reactiver`)
export const updateProjetOrdre   = (id, ordre)     => client.patch(`/projets/${id}/ordre`, { pages_ordre: ordre })
